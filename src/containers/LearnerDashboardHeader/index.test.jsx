...
describe('LearnerDashboardHeader', () => {
  test('render', () => {
    ...
    expect(wrapper.instance.findByType(HeaderSlot)).toHaveLength(1);
    wrapper.instance.findByType(HeaderSlot)[0].props.mainMenuItems[1].onClick();
    expect(findCoursesNavClicked).toHaveBeenCalledWith(urls.baseAppUrl('/course-search-url'));
    expect(wrapper.instance.findByType(HeaderSlot)[0].props.secondaryMenuItems.length).toBe(0);
  });

  test('should display Help link if SUPPORT_URL is set', () => {
    mergeConfig({ SUPPORT_URL: 'http://localhost:18000/support' });
    const wrapper = shallow(<LearnerDashboardHeader />);
    expect(wrapper.instance.findByType(HeaderSlot)[0].props.secondaryMenuItems.length).toBe(1);
  });
  test('should display Programs link if it is enabled by configuration', () => {
    mergeConfig({ ENABLE_PROGRAMS: true });
    const wrapper = shallow(<LearnerDashboardHeader />);
    expect(wrapper.instance.findByType(HeaderSlot)[0].props.mainMenuItems.length).toBe(3);
  });
});
